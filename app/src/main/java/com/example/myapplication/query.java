        ...
        try {
            ...
            for(int i=0;i<jsonArray.length();i++){
               ...
               // Log.e(LOG_TAG,"author test "+link);
                ...
                Log.e(LOG_TAG,"author test "+image);*/

                JSONObject imageLink = volume.getJSONObject("imageLinks");
                String image = "";
                if (imageLink.has("smallThumbnail")) {
                    String originalImageUrl = imageLink.getString("smallThumbnail");

                    // Check if the original URL is in HTTP format
                    if (originalImageUrl.startsWith("http://")) {
                        // Convert it to HTTPS
                        image = originalImageUrl.replace("http://", "https://");
                    } else {
                        // The URL is already in HTTPS format or some other format
